...
// Registrar Widget de Destaque solo invertido
...
class WidgetDestaqueSoloInvertido extends WP_Widget {
    ...
    public function __construct() {
        parent::__construct(
            ...
            array(
                'description' => 'Destaca uma página do site de forma belíssima, só que com a imagem no lado direito.'
            )
        );
    }
                ...
                <div class="flex-grow">
                    ...
                    for ($i = 1; $i < 6; $i++){
                        if(!empty($instance['pagina_link'][$i]) && !empty($instance['link_texto'][$i])) {
                            ...
                            if (in_array( wp_parse_url( $instance['pagina_link'][$i], PHP_URL_HOST ), wp_internal_hosts(), true )) {
